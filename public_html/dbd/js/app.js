...
const DbDApp = {
  ...
  renderPerks(perks, container) {
    ...
    container.innerHTML = Object.entries(groups).map(([charName, charPerks]) => {
      ...
      const charIcon = imgPath
        ? `<img src="${escapeHtml(imgPath)}" alt="${escapeHtml(charName)}" class="char-icon-img" loading="lazy">`
        : `<span style="width:40px;height:40px;border-radius:50%;border:2px solid var(--border);background:var(--bg-secondary);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:1.3rem;">${this.getPerkIcon(charPerks[0].role)}</span>`;
      return `
  ...
  renderKillerAbilities(killers, container) {
    container.innerHTML = killers.map(k => `
      <div class="card" style="margin-bottom:16px;">
        <div style="display:flex;gap:16px;align-items:flex-start;">
          ${k.image_path ? `<img src="${escapeHtml(k.image_path)}" alt="${escapeHtml(k.name)}" style="width:80px;height:80px;border-radius:8px;object-fit:contain;flex-shrink:0;" loading="lazy">` : ''}
          <div style="flex:1;">
    ...
    container.innerHTML = addons.map(a => `
      <div class="card" style="margin-bottom:12px;">
        <div style="display:flex;gap:12px;align-items:flex-start;">
          ${a.image_path ? `<img src="${escapeHtml(a.image_path)}" alt="${escapeHtml(a.name)}" style="width:56px;height:56px;border-radius:6px;object-fit:contain;flex-shrink:0;" loading="lazy">` : ''}
          <div style="flex:1;min-width:0;">
            ...
            <div class="subtitle">${escapeHtml(a.killer_name || '')}</div>
