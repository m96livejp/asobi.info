...
try {
    ...
    $recipeNo = $_GET['recipe_no'] ?? null;
    $pokemonByRecipe = $_GET['pokemon_by_recipe'] ?? null;

    if ($pokemonByRecipe && $recipeNo) {
        $stmt = $db->prepare('
            SELECT DISTINCT p.pokedex_no, p.name, p.type1, p.type2, p.base_hp, p.base_atk, p.ranged,
                   p.quality_normal, p.quality_good, p.quality_great, p.quality_special
            FROM recipe_pokemon rp
            JOIN recipes r ON rp.recipe_id = r.id
            JOIN pokemon p ON rp.pokemon_id = p.pokedex_no
            WHERE r.recipe_no = ?
            ORDER BY p.pokedex_no
        ');
        $stmt->execute([(int)$recipeNo]);
    ...
    if ($grouped) {
        ...
        $ingMap = [];
        $ingRows = $db->query('
            SELECT ri.recipe_no, i.id, i.name, i.image_path, i.color, i.softness, i.size, i.category, ri.condition_group
