...
// SD API ヘルスチェック（/sdapi/v1/options）
$sdCheckUrl = $sdEndpoint ? rtrim($sdEndpoint, '/') . '/sdapi/v1/options' : '';

// LibreTranslate ヘルスチェック（/languages）
$ltCheckUrl = $ltEndpoint ? rtrim($ltEndpoint, '/') . '/languages' : '';

// Ollama ヘルスチェック（/api/version またはbaseURL）
$ollamaCheckUrl = '';
if ($ollamaEndpoint) {
    // /api/generate のベースURLから /api/version を組み立てる
...
<div class="api-grid">
  ...
  <div class="api-card">
    ...
    <div class="check-result" id="result-sd"></div>
    <?php else: ?>
  ...
  <div class="api-card">
    ...
    <div class="check-result" id="result-ollama"></div>
    <?php else: ?>
    <span style="font-size:0.8rem;color:#aaa;">エンドポイント未設定のためテスト不可</span>
    <?php endif; ?>
  </div>
  ...
  <div class="api-card">
    ...
    <div class="check-result" id="result-lt"></div>
    <?php else: ?>
  ...
  <div class="api-card">
    ...
    <div class="check-result" id="result-vox"></div>
  </div>
...
async function checkApi(btn, url) {
    ...
    try {
        ...
        if (data.ok) {
            resultEl.textContent = '接続OK (' + data.ms + 'ms)';
        } else {
