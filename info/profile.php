...
$pwError = $pwSuccess = '';
$ageError = $ageSuccess = '';
// OAuth年齢認証コールバックのメッセージ処理
if (!empty($_GET['age_verified'])) {
    $ageSuccess = '年齢認証が完了しました（Googleの生年月日で確認済み）';
    // セッションキャッシュも更新
    $_SESSION['asobi_age_verified'] = true;
    // プロフィールを最新状態で再読み込み
    $profile = $db->query("SELECT * FROM users WHERE id = " . (int)$user['id'])->fetch();
}
if (!empty($_GET['age_verify_error'])) {
    $errType = $_GET['age_verify_error'];
    if ($errType === 'no_birthday')  $ageError = 'Googleアカウントに生年月日が登録されていません。Googleアカウントの設定で生年月日を登録してから再度お試しください。';
    elseif ($errType === 'underage') $ageError = '18歳未満のため年齢認証はできません。';
    else                             $ageError = '年齢認証に失敗しました。再度お試しください。';
}
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .btn-google-age-verify:hover { border-color: #4285F4; box-shadow: 0 2px 8px rgba(66,133,244,0.18); }
...
<body>
  ...
  <?php
    $back = $_GET['back'] ?? '';
    // サブドメインのみ許可
    if ($back && preg_match('#^https://[a-z]+\.asobi\.info/?$#', $back)):
  ?>
  <div style="max-width:900px;margin:12px auto 0;padding:0 20px;">
    <a href="<?= htmlspecialchars($back) ?>" style="font-size:0.85rem;color:#667eea;text-decoration:none;">← <?= htmlspecialchars(parse_url($back, PHP_URL_HOST)) ?> に戻る</a>
  </div>
  <?php endif; ?>
    ...
    <main class="profile-main">
      ...
      <div class="section-card" id="age-verify">
        ...
        <?php else: ?>
          ...
          <p style="font-size:0.88rem;color:#4b5563;line-height:1.75;margin-bottom:16px;">
            Googleアカウントに登録済みの生年月日を使って年齢確認を行います。<br>
            Googleアカウントを持っていない場合は、先に連携してください。
          </p>
          <?php
            $hasGoogle = false;
            foreach ($socialAccounts as $sa) {
                if ($sa['provider'] === 'google') { $hasGoogle = true; break; }
            }
            $ageVerifyRedirect = !empty($_GET['redirect']) ? $_GET['redirect'] : '';
            $ageVerifyUrl = '/oauth/start.php?provider=google&mode=age_verify'
                          . ($ageVerifyRedirect ? '&redirect=' . urlencode($ageVerifyRedirect) : '');
          ?>
          <a href="<?= htmlspecialchars($ageVerifyUrl) ?>" class="btn-google-age-verify">
            ...
            Googleで生年月日を確認して年齢認証
          </a>
          <p class="hint" style="margin-top:10px;">※ Googleアカウントに生年月日が登録されており、18歳以上の場合のみ認証できます。</p>
        <?php endif; ?>
      </div>
