 ...
 *
 * データソース:
 *   GitHub game-soft/mega-drive (soft_list.json)
 *
 * 使い方:
 *   php import-megadrive.php                  # インポート実行
 *   php import-megadrive.php --dry-run        # 実行せず内容を確認
 *   php import-megadrive.php --reset          # 既存MDデータを削除して再投入
 */
...
if ($reset && !$dryRun) {
    $deleted = $pdo->exec("DELETE FROM games WHERE platform = 'md'");
    echo "既存MDデータを削除しました: {$deleted}件\n";
}

// --- GitHubデータ取得 ---
$jsonUrl = 'https://raw.githubusercontent.com/game-soft/mega-drive/master/data/soft_list.json';
echo "データ取得中: {$jsonUrl}\n";
...
$total = $pdo->query("SELECT COUNT(*) FROM games WHERE platform = 'md'")->fetchColumn();
echo "  MD合計: {$total}件\n";
